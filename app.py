import os, io, re, base64, time, json
...
# FORZADO: Usamos la URL base estándar para eliminar variables
OPENAI_BASE_URL = "https://api.openai.com/v1" 
EMBED_MODEL = "text-embedding-ada-002" # Modelo estable y compatible
...
# ... (El resto de las funciones de ayuda van aquí, no se modifican)
...
# ================== OpenAI Wrappers (CON MANEJO DE ERRORES MEJORADO) ==================
def openai_embed(texts: List[str]) -> np.ndarray:
    ...
    try:
        ...
        # Extrae el cuerpo del error de OpenAI y lo relanza
        error_details = e.response.text
        raise Exception(f"Error HTTP de OpenAI: {e.response.status_code} - {error_details}") from e
...
# ... (El resto de las funciones de pdf_to_images y ocr_images_with_openai no se modifican)
...
def ocr_images_with_openai(images: List[bytes]) -> str:
    ...
    for i in range(0, len(images), batch):
        ...
        content = [{"type": "text", "text": "Extrae el texto legible de estas páginas en orden. Devuelve solo TEXTO PLANO."}]
        for img in group:
...
def health():
    return text_response("✅ EFFICON Analyzer activo (v. robusta).")

@app.route("/api/analyze", methods=["POST"])
def analyze():
    # ... (El código de la ruta 'analyze' no necesita cambios, usará las funciones mejoradas)
    start_time = time.time()
    print(f">>> [{start_time}] Petición de análisis recibida.")
